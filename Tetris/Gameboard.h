...
public:	
	...
	void setContent(const Point &point, int val);
	...
	// - return: nothing
	void setContent(int x, int y, int val);
	...
	// - return: nothing
	void setContent(std::vector<Point> &blocks, int val);
	...
	bool areAllLocsEmpty(std::vector<Point> &checks) const; 
...
private:  // This is commented out to allow us to test. 
	...
	bool isValidPoint(Point const &point) const;
